...
#提前设置位置具体点位
def xy_get(name,path,title):
    # 定义一个映射，用于将 names 映射到对应的 x, y, name 值
    
    if title == '华睿稳健FOF一号':
        map = {
            '华睿稳健FOF一号': (0, -1),
            #'杉阳云杉量化1号私募证券投资基金': (3, -1),
            #'跃威权鑫一号私募证券投资基金A类': (3, -1),
            #'珺容无相纯债2号私募证券投资基金': (1, -1),
            #'鸣熙致远二十号私募证券投资基金': (3, -1),
            #'远澜银杏1号私募证券投资基金': (2, -1),
            #'潼骁新资产私募证券投资基金': (3, -1),
            #'蒙玺科创50指数量化1号私募证券投资基金B': (2, -1),
            #'殊馥兴义2号私募证券投资基金': (3, -1),
            #'致远量化对冲运作6号私募证券投资基金B类': (1, 0),
            '量派对冲9号私募证券投资基金A': (3, -1),
            #'鸣熙CTA2号私募证券投资基金': (3, -1),
            #'吾执九二号私募证券投资基金B类': (3, -1),
            #'宏翼高频1号私募证券投资基金': (3, -1),
            '量派CTA七号私募证券投资基金A': (3, -1),
            #'鸣熙日耀套利1号私募证券投资基金': (3, -1),
            '博润易量稳健1号私募证券投资基金': (4, -1),
            #'世纪前沿量化对冲62号a期私募证券投资基金B': (3, -1),
            '交叉智能-量化增强2号私募证券投资基金': (3, -1),
            #'波粒二象东大楼私募证券投资基金':(3,-1)
        ...
        #'致远量化对冲运作6号私募证券投资基金B类': (1, 0),
        #'跃威常鑫一号私募证券投资基金':(0,0),
        #'优美利东海心选1号私募证券投资基金':(3, -1),
        #'汇艾资产稳健2号私募证券投资基金':(3, -1),
        #'吾执九二号私募证券投资基金B类': (3, -1),
        '世纪前沿量化对冲62号a期私募证券投资基金B': (3, -1),
        '灵均恒君3号私募证券投资基金': (2, -1)
    }
        ...
        #'跃威常鑫一号私募证券投资基金':(0,0),
        #'殊馥兴义2号私募证券投资基金': (3, -1),
        #'千衍九凌1号私募证券投资基金B类':(1,-1),
        '量派CTA七号私募证券投资基金A':(3,-1),
        '量派对冲9号私募证券投资基金A': (3, -1),
        # '宏翼高频1号私募证券投资基金': (3, -1),
        '常瑜宇称2号私募证券投资基金': (0, -1),
        '东恺祖率中性一号私募证券投资基金': (0, -1)
    }
...
def get_ht(names,x,y,path):
    if  names == '东恺祖率中性一号私募证券投资基金'or names == '常瑜宇称2号私募证券投资基金':
        return
    else:
...
#获取所需数据
def get_html(names,x,y,path):
    last_num = get_ht(names=names,x=x,y=y,path=path)
    if names == '东恺祖率中性一号私募证券投资基金'or names == '常瑜宇称2号私募证券投资基金':
        read_download_file(path)
    else:
        with IMAPClient(HOST) as client:
            # 登录到IMAP服务器
            client.login(USERNAME, PASSWORD)
        ...
        if first_date:
            ...
            if first_date:
                ...
                # 然后，使用字符串切片和拼接来改变格式
                df['日期'] = df['日期'].str.slice(0, 4) + '-' + df['日期'].str.slice(4, 6) + '-' + df['日期'].str.slice(6, 8)
                print('子基金:'+names+'日期是：'+df['日期'][0]+'累计净值是：'+df_num['累计净值'][x])
                # Assign the data to the DataFrame
                df['子基金名称'] = [names]  # Make sure 'names' is a list or wrapped in a list if it's a single value
                df['累计净值'] = [df_num['累计净值'][x]]  # Wrap the value in a list to ensure it is compatible with DataFrame assignment
...
def combine(a,b,c):
    ...
    names, x, y, path = xy_get(name, path, title)
    get_html(names, x, y, path)
#三个fof的各个子基金跑一遍
def fof1():
    combine('华睿稳健FOF一号','华睿稳健FOF一号',first)
    #combine('杉阳云杉量化1号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('跃威权鑫一号私募证券投资基金A类','华睿稳健FOF一号',second)
    #combine('珺容无相纯债2号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('盛丰烁今1号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('鸣熙致远二十号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('远澜银杏1号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('潼骁新资产私募证券投资基金','华睿稳健FOF一号',second)
    #combine('蒙玺科创50指数量化1号私募证券投资基金B','华睿稳健FOF一号',second)
    #combine('殊馥兴义2号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('致远量化对冲运作6号私募证券投资基金B类','华睿稳健FOF一号',second)
    combine('量派对冲9号私募证券投资基金A','华睿稳健FOF一号',second)
    #combine('鸣熙CTA2号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('吾执九二号私募证券投资基金B类','华睿稳健FOF一号',second)
    combine('量派CTA七号私募证券投资基金A','华睿稳健FOF一号',second)
    #combine('宏翼高频1号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('鸣熙日耀套利1号私募证券投资基金','华睿稳健FOF一号',second)
    combine('博润易量稳健1号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('世纪前沿量化对冲62号a期私募证券投资基金B','华睿稳健FOF一号',second)
    combine('交叉智能-量化增强2号私募证券投资基金','华睿稳健FOF一号',second)
    #combine('波粒二象东大楼私募证券投资基金','华睿稳健FOF一号',second)
def fof2():
    combine('匠燃进取FOF','匠燃进取FOF',first)
    #combine('常瑜宇称2号私募证券投资基金','匠燃进取FOF',second)
    #combine('汇艾资产稳健2号私募证券投资基金','匠燃进取FOF',second)
    #combine('量派对冲9号私募证券投资基金A','匠燃进取FOF',second)
    #combine('殊馥兴义2号私募证券投资基金','匠燃进取FOF',second)
    #combine('潼骁新资产私募证券投资基金','匠燃进取FOF',second)
    combine('灵均恒君3号私募证券投资基金','匠燃进取FOF',second)
    combine('浅湾-岳海精选六号股指CTA专享私募证券投资基金','匠燃进取FOF',second)
def fof3():
    combine('华凌轩哥六号','华凌轩哥六号',first)
    #combine('图斯和贞私募证券投资基金','华凌轩哥六号',second)
    #combine('潼骁周周享私募证券投资基金','华凌轩哥六号',second)
    #combine('致远量化对冲运作6号私募证券投资基金B类','华凌轩哥六号',second)
    #combine('跃威常鑫一号私募证券投资基金','华凌轩哥六号',second)
    #combine('优美利东海心选1号私募证券投资基金','华凌轩哥六号',second)
    #combine('汇艾资产稳健2号私募证券投资基金','华凌轩哥六号',second)
    #combine('吾执九二号私募证券投资基金B类','华凌轩哥六号',second)
    combine('世纪前沿量化对冲62号a期私募证券投资基金B','华凌轩哥六号',second)
    combine('灵均恒君3号私募证券投资基金','华凌轩哥六号',second)
def fof4():
    combine('华云琢玉FOF一号','华云琢玉FOF一号',first)
    #combine('鸣熙致远二十号','华云琢玉FOF一号',second)
    #combine('潼骁新资产私募证券投资基金','华云琢玉FOF一号',second)
    # combine('珺容无相纯债2号私募证券投资基金','华云琢玉FOF一号',second)
    #combine('跃威常鑫一号私募证券投资基金','华云琢玉FOF一号',second)
    #combine('殊馥兴义2号私募证券投资基金','华云琢玉FOF一号',second)
    #combine('千衍九凌1号私募证券投资基金B类','华云琢玉FOF一号',second)
    combine('量派CTA七号私募证券投资基金A','华云琢玉FOF一号',second)
    combine('量派对冲9号私募证券投资基金A','华云琢玉FOF一号',second)
    # combine('宏翼高频1号私募证券投资基金','华云琢玉FOF一号',second)
    combine('常瑜宇称2号私募证券投资基金','华云琢玉FOF一号',second)
    combine('东恺祖率中性一号私募证券投资基金','华云琢玉FOF一号',second)
#网络文字解码
...
#找到各个附件的数据位置并爬取储存
def read_download_file(path):
    if path == 'INBOX/华凌轩哥六号/致远量化对冲运作6号私募证券投资基金B类':
        x = 1
        y = 7
        z = 2
        s = 1
    if path == 'INBOX/华云琢玉FOF一号/东恺祖率中性一号私募证券投资基金':
        x = 0
        y = 7
        z = 10
        s = 3
        ...
        s = 0
    download_file(path)
    df=pd.read_excel('C:/Users/Administrator/Desktop/temp.xls')
    ...
    if '常瑜宇称2号私募证券投资基金（A类份额）' in df.iloc[s, x]:
